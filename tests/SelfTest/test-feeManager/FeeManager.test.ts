...
describe('receiveCallFromComptroller', () => {
  it('calls the correct action for actionId', async () => {
    ...
    const investmentAmount = utils.parseUnits('100', await denominationAsset.decimals());
    ...
    await mockContinuousFeeSettleOnly.settle.returns(
      ...
      utils.parseUnits('5', await denominationAsset.decimals()),
    );
...
describe('__payoutSharesOutstandingForFees', () => {
  it('pays out shares outstanding (if payable) and emits one event per payout', async () => {
    ...
    const feeAmount1 = utils.parseUnits('50', await denominationAsset.decimals());
    const feeAmount2 = utils.parseUnits('25', await denominationAsset.decimals());
    const feeAmount22 = feeAmount2.sub(feeAmount2.mul(8).div(100));
