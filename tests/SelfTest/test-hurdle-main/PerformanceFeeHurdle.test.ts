...
async function assertAdjustedPerformance({
  ...
  Number(BigNumber.from(sharesDue))); //0.049868766404199470*10**18  0.052631578947368424
  ...
  console.log("=====LastSharePriceUpdated::", 
  Number(BigNumber.from(feeInfo.lastSharePrice)), //10**18
...
describe('integration', () => {
  it('works correctly upon shares redemption for a non 18-decimal asset', async () => {
    ...
    Number(BigNumber.from(gavPostRedeem1))); //50*10**18
    ...
    Number(BigNumber.from(gavPostRedeem2)));//25*10**18
