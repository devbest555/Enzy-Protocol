...
async function snapshot() {
  ...
  const createFund = () => {
    const feesSettingsData = [utils.randomBytes(10), utils.randomBytes(10), utils.randomBytes(2), constants.HashZero, utils.randomBytes(2)];
...
describe('__payoutSharesOutstandingForFees', () => {
  it('pays out asset outstanding (if payable) and emits one event per payout', async () => {
    const {
      accounts: [buyer],
      deployment,
      fees: { mockContinuousFeeSettleOnly, mockStreamingFeeSettle, mockContinuousFeeWithGavAndUpdates, mockHurdleFeeSettle },
      fundOwner,
      deployer,
      denominationAsset,      
      createFund,
    ...
    Number(BigNumber.from(gav_1))); //0.998*10**18


    // Define both fees the same way, but with different fee amounts     
    const shareManagement = utils.parseEther('0.1');//10% managementFee
    await mockContinuousFeeSettleOnly.settle.returns(FeeSettlementType.MintSharesOutstanding, constants.AddressZero, shareManagement);
    
    const shareStreaming = utils.parseEther('0.05');//5% streamingFee
    await mockStreamingFeeSettle.settle.returns(FeeSettlementType.MintSharesOutstanding, constants.AddressZero, shareStreaming);
    ...
    await mockContinuousFeeWithGavAndUpdates.settle.returns(FeeSettlementType.MintSharesOutstanding, constants.AddressZero, shareHWM);
    ...
    await mockHurdleFeeSettle.settle.returns(FeeSettlementType.MintSharesOutstanding, constants.AddressZero, shareHurdle);
    
    // Define param for all calls on extension
    const extension = deployment.feeManager;
    const fees = [mockContinuousFeeSettleOnly, mockStreamingFeeSettle, mockContinuousFeeWithGavAndUpdates, mockHurdleFeeSettle];
    ...
    Number(BigNumber.from(postShareOutstandingCall)),//0
    Number(BigNumber.from(gav))); //0.998*10**18
